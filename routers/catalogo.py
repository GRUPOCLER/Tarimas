...
def _normalizar_col(nombre: str) -> str:
    ...
    n = _re.sub(r"\([^)]*\)", "", n).strip()  # quita "(kg)", "(cm)", etc.
    n = _re.sub(r"\s+", " ", n)
...
async def importar_catalogo(
    ...
    await db.commit()
    return {
        "ok": True, "insertados": insertados, "actualizados": actualizados,
        "omitidos": omitidos, "total_filas": len(filas),
        "hoja_usada": ws.title if not nombre.endswith(".csv") else "(csv)"
    }
